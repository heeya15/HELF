...
import { 
    ...
    DietDiaryItem, 
    TotalKcal,
    DeleteButton } from './MyDiet.style';
...
export default function MyDietDaily() {
    ...
    const { myDietDiaryDailyInfo, myDietDiaryDeleteDone } = useSelector(state => state.myDietRegister);
    ...
    for(let i=0; i<myDietDiaryDailyInfo.length; i++) {
        ...
        const diaryTime = myDietDiaryDailyInfo[i].diaryDate.substring(11, 16) + " " + (hour < 12 ? "AM" : "PM");
            ...
            <div>
                {
                    diaryInfoList.map(info => (
                        <DietDiaryItem key={info.diaryNo}>
                            ...
                            <ShareButton
                                onClick={ () => clickShareBtn(info.diaryNo) }
                            >Share</ShareButton>
