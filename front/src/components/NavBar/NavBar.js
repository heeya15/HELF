...
import {LOG_IN_SUCCESS, LOG_OUT_REQUEST, SET_MENU} from '../../store/modules/user';
            ...
            <Container>
                ...
                <Navbar.Collapse id="responsive-navbar-nav">
                    <Nav className="me-auto">
                        <Nav.Link onClick={ handleShareBoard } className='anc'>게시판</Nav.Link>      
                        <Nav.Link onClick={ handleMyDiet } className='anc'>MY식단</Nav.Link>        
                        <Nav.Link onClick={ handleMyExercise } className='anc'>MY운동</Nav.Link>        
                    </Nav>
                    <Nav>
                        {(logInDone ===false && kakaologInDone === false) &&
                        <Nav.Link onClick={ handleSignUp } className='anc'><span >회원가입</span></Nav.Link>      
                        }
                        {(logInDone ===false && kakaologInDone === false) &&
                        <Nav.Link onClick={ handleLogIn } className='anc'>로그인</Nav.Link>  
                        }
                        {(logInDone ===true || kakaologInDone === true) &&
                        <Nav.Link onClick={ handleMyPage } className="anc"> MY페이지 </Nav.Link>
                        }
                        {(logInDone === true || kakaologInDone === true) &&
                        <Nav.Link
                            className='anc'
